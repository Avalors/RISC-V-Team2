...
for file in "${files[@]}"; do #loops through all elements in the files array
    name=$(basename "$file" _tb.cpp | cut -f1 -d\-) #Extracts the "base name" of the file, stripping the _tb.cpp suffix and anything after the first - in the name
    
    # If verify.cpp -> we are testing the top module
    #if [ $name=="verify.cpp" ]; then
    #    name="top" #rename name to top
    #fi
...
if [ $fails -eq 0 ]; then
    ...
    exit 0
# If there were any failures (fails > 0), the script:
# Calculates the total number of tests by summing passes and fails.
# Prints a failure message in red using the ${RED} formatting variable.
# Exits with a status of 1 (failure)
else
    total=$((passes + fails))
